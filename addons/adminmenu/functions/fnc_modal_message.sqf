...
private _ctrlEditPos = [0.1 * OP183_ADMINMENU_STD_WIDTH, 0.1 * OP183_ADMINMENU_STD_HEIGHT, _ctrlGrpWidth - (0.2 * OP183_ADMINMENU_STD_WIDTH), _ctrlGrpHeight - (1.3 * OP183_ADMINMENU_STD_HEIGHT)];
...
_ctrlCombo ctrlSetPosition [0.1 * OP183_ADMINMENU_STD_WIDTH, _ctrlGrpHeight - OP183_ADMINMENU_STD_HEIGHT, _ctrlGrpWidth * 0.3, OP183_ADMINMENU_STD_HEIGHT];
...
_ctrlButtonPreview ctrlSetPosition [(_ctrlGrpWidth * 0.7) - (0.2 * OP183_ADMINMENU_STD_WIDTH), _ctrlGrpHeight - OP183_ADMINMENU_STD_HEIGHT, (_ctrlGrpWidth * 0.15), OP183_ADMINMENU_STD_HEIGHT];
...
_ctrlButtonPreview ctrlAddEventHandler ["buttonClick", {
    ...
    if (_editText isEqualTo "") then {
        systemChat "[OP183 Admin Menu] Message can't be empty";
    } else {
        ...
        if (_venue isEqualTo QFUNC(showSubtitle)) then {
            ...
            if (_venue isEqualTo "hint") then {
                ...
                systemChat format ["[OP183 Admin Message] %1", _editText];
            };
        };

        systemChat "[OP183 Admin Menu] Message previewed";
    };
}];

private _ctrlButtonCommit = _display ctrlCreate [QGVAR(RscButtonMenu), -1, _ctrlGroup];
_ctrlButtonCommit ctrlSetPosition [(_ctrlGrpWidth * 0.85), _ctrlGrpHeight - OP183_ADMINMENU_STD_HEIGHT, (_ctrlGrpWidth * 0.15) - (0.1 * OP183_ADMINMENU_STD_WIDTH), OP183_ADMINMENU_STD_HEIGHT];
...
_ctrlButtonCommit ctrlAddEventHandler ["buttonClick", {
    ...
    if (_editText isEqualTo "") then {
        ...
        if (_venue isEqualTo QFUNC(showSubtitle)) then {
            ...
            (format ["[OP183 Admin Message] %1", _editText]) remoteExec [_venue, GVAR(utilityData)];
        };

        systemChat "[OP183 Admin Menu] Message sent to player(s)";
