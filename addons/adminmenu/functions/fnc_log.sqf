...
/* ----------------------------------------------------------------------------
Function: OP183_adminmenu_fnc_log
...
Parameters:
    ...
    _tag - Tag that message will be prefixed with [String, defaults to "[OP183 Log]"]
...
Examples:
    (begin example)
        ["Everything is on fire!",true,"[OP183 Firealarm]"] call OP183_adminmenu_fnc_log;
    (end)
    (begin example)
        [format ["%1 did something",profileName]] call OP183_adminmenu_fnc_log;
    (end)
