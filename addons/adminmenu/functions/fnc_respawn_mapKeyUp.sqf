...
if (_type == 28) then {
    if (GVAR(respawnMousePos) isEqualTo -1) then {
        ...
        if (GVAR(respawnMousePos) isEqualType []) then {
            ...
            private _var = missionNamespace getVariable[format["op183_respawnPoint%1",GVAR(respawnMousePos)],objNull];
            if (!isNull _var) then {
        ...
        // Close the Map
        ((findDisplay IDC_OP183_ADMINMENU_RESP_MAP_DISPLAY) displayCtrl IDC_OP183_ADMINMENU_RESP_MAP_CONTROL) ctrlShow false;
        ((findDisplay IDC_OP183_ADMINMENU_RESP_MAP_DISPLAY) displayCtrl IDC_OP183_ADMINMENU_RESP_MAP_CONTROL) mapCenterOnCamera false;
        closeDialog IDC_OP183_ADMINMENU_RESP_MAP_DISPLAY;
    };
};
