/*
Function name:fnc_Onc_AmmoCrate;
Author: Mr H.
...
Example(s):
[this,10,true,true] call fnc_AmmoCrate;
*/
...
_box setVariable ["AmmoMagazines", _allMagazinesType];

_handlerIndex = _box addEventHandler
 ["ContainerOpened",
  {
  _box = _this select 0;
  _allMagazinesType = _box getVariable "AmmoMagazines";



  clearMagazineCargoGlobal _box;
  {_box addMagazineCargoGlobal [_x,10];} forEach _allMagazinesType;
        _box addItemCargoGlobal ["medbags_FirstAid",1];
        _box addItemCargoGlobal ["medbags_MedicKit",1];
        _box addItemCargoGlobal ["mopp",1];
  _box addItemCargoGlobal ["HandGrenade",8];
  _box addItemCargoGlobal ["SmokeShell",8];
  _box addItemCargoGlobal ["ACE_M84",8];
  }
 ];
_box setVariable ["AmmoCrateEHIndex", _handlerIndex, true];
...
_box setVariable ["AmmoCrateParameters", _params, true];
...
_statement =
 ...
 _box = _this select 0;
 _params = _box getVariable "AmmoCrateParameters";
 [[_params],
  {
  params ["_params"];
  _box = _params select 0;
  _handlerIndex = _box getVariable "AmmoCrateEHIndex";
