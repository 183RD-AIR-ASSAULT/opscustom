/*
Function name:OP183_fnc_OP183_fnc_AmmoCrate;
Author: Mr H.
...
Example(s):
[this,10,true,true] call OP183_fnc_AmmoCrate;
*/
...
_box setVariable ["OP183_AmmoMagazines", _allMagazinesType];

_handlerIndex = _box addEventHandler
 ["ContainerOpened",
  {
  _box = _this select 0;
  _allMagazinesType = _box getVariable "OP183_AmmoMagazines";



  clearMagazineCargoGlobal _box;
  {_box addMagazineCargoGlobal [_x,10];} forEach _allMagazinesType;
        _box addItemCargoGlobal ["OP183_medbags_FirstAid",1];
        _box addItemCargoGlobal ["OP183_medbags_MedicKit",1];
        _box addItemCargoGlobal ["OP183_mopp",1];
  _box addItemCargoGlobal ["HandGrenade",8];
  _box addItemCargoGlobal ["SmokeShell",8];
  _box addItemCargoGlobal ["ACE_M84",8];
  }
 ];
_box setVariable ["OP183_AmmoCrateEHIndex", _handlerIndex, true];
...
_box setVariable ["OP183_AmmoCrateParameters", _params, true];
...
_statement =
 ...
 _box = _this select 0;
 _params = _box getVariable "OP183_AmmoCrateParameters";
 [[_params],
  {
  params ["_params"];
  _box = _params select 0;
  _handlerIndex = _box getVariable "OP183_AmmoCrateEHIndex";
  ...
  _params call OP183_fnc_AmmoCrate;
  }
